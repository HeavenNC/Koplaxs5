...
cl = LINETCR.LINE() #Captain
#cl.login(qr=True)
cl.login(token='Eq7cRGnm3ut2kibDs3K2.WuAN6ZMYbnj1pDJejZ3XeG.9YnKQhsX6ml3Hf5Rl41hDc40/UGvyecFIA9+9jekdK0=')
cl.loginResult()

ki = LINETCR.LINE() #Monika
#ki.login(qr=True)
ki.login(token='EqJxjfSjCNFCFkrfzuT3.f7PFfPKpsK3A22938LA0mW.ermIXjsq8bz795LLXYnUVlUJdsPpOzX0CCTaTZwz0u0=')
ki.loginResult()

kk = LINETCR.LINE() #Sayori
#kk.login(qr=True)
kk.login(token='Eqf5lB3sSuUYWYPar3N2.qP1HYMYKPSKmjxf+rX88aG.e6V0jmNIBBjnbXUqOFeASYoLDBkWt5ZFlU6ra8mAkDw=')
kk.loginResult()

kc = LINETCR.LINE() #Yuri
kc.login(token='EqkeL0QtAUmwsbKbi0zd.T+LoDe0A6jcy2jWFfEEhJq.ZAy7ABrXS2dvQyd73sor713mOLYj9/TAhqWnl8bRuRU=')
kc.loginResult()

ks = LINETCR.LINE() #Natsuki
#ks.login(qr=True)
ks.login(token='EqzbArzRudbss38fsIC5.WGwa6C5aM1+nmYjdE9JMjq.fZpV5HvUz2BZDded4dsNvs+8uhiBkj8kOM+aV8lKEJ0=')
ks.loginResult()
...
helpMessage ="""M҉o҉n҉i҉k҉a ҉T҉e҉a҉m҉ ҉P҉r҉o҉t҉e҉c҉t҉
-==================-
◄]·♦·Menu For Public·♦·[►
[•]Adminlist
[•]Ownerlist
...
👑Hanya Untuk Owner👑
-==================-
M҉o҉n҉i҉k҉a ҉T҉e҉a҉m҉ ҉P҉r҉o҉t҉e҉c҉t҉
-==================-
"""
KAC=[cl,ki,kk,kc,ks]
...
≫ Idline: http://line.me/ti/p/~syifaabyananta ≪""",
    "lang":"JP",
...
#-----------------------------------------------
         #----------------Fungsi Join Group Start-----------------------#
            elif msg.text in ["Monika bot"]: #Panggil Semua Bot
              if msg.from_ in owner:
    ...
    #-------------Fungsi Leave Group Start---------------#
            ...
            elif msg.text in ["Monika bye"]: #Semua Bot Ninggalin Group Kecuali Bot Induk
              if msg.from_ in admin:
                if msg.toType == 2:
                    ginfo = cl.getGroup(msg.to)
                    try:
...
#-----------------------------------------------
       ...
       #-------------Fungsi Respon Start---------------------#
              ...
              if msg.from_ in admin:
                cl.sendText(msg.to,"Captain Siap")
                ki.sendText(msg.to,"Monika Siap")
                kk.sendText(msg.to,"Sayori Siap")
                kc.sendText(msg.to,"Yuri Siap")
                ks.sendText(msg.to,"Natsuki Siap")
        ...
        if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
            if hasil['result']['posts'][zx]['userInfo']['mid'] in owner:
                try:
                    ...
                    cl.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"Auto like by ^Monika Bot^\nStatus Boss udah Kami Like\nOwner Kami :\nMonika")
                    print "Like"
                except:
                    pass
            else:
